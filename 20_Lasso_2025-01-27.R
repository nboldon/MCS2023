...
## Scatterplots, Pearson's R, and the Lasso
...
merged_data <- merged_data %>% drop_na(Log2FC, all_of(behavior_metrics))
...
# If still encountering issues, filter out rows with non-finite values explicitly
merged_data <- merged_data %>%
  filter(across(c(Log2FC, all_of(behavior_metrics)), ~ is.finite(.)))
...
## Results in duplicated behavioral results in the merged spreadsheet
