...
## Updated color to viridis 12/16/2024
...
p <- ggplot(cluster_counts_long, aes(x = factor(cluster), y = count, fill = group)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("g1" = "#440154", "g2" = "#35B779FF"),  # Custom colors for the groups
                    labels = c("2N/2N+", "Ts/Ts+")) +  # Customize the legend labels
  labs(x = "Cluster", 
       y = "Total Gene Count",  # Update y-axis label
       fill = "Group", 
       title = "Total Gene Count by Cluster for Genotype Pairwise Comparisons") +  # Add plot title
...
# Save the plot as a .jpg file
ggsave("/Volumes/DataBox/Heatmap_Comps/genotyp_gene-counts_byCluster_Barplot_2024-12-16.jpg", plot = p, width = 10, height = 6)
