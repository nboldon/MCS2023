...
#sample_combinations <- expand.grid(unique(projMCS5$Sample), unique(projMCS5$Sample))

# find the rows where the first and second columns are the same sample
#rows_remove <- vector()
#for(i in 1:nrow(sample_combinations)){
#  if(sample_combinations[i,1] == sample_combinations[i,2]){
#    rows_remove <- c(rows_remove, i)
#  }
#}
# make a new, non-redundant object of the combinations
#non_redundant_combs <- sample_combinations[-rows_remove,]

#for(j in unique(projMCS5$Clusters)[1:25]){ # if testing on just first 3 clusters


####

C18ArchRSubset <- projMCS5[projMCS5$Clusters %in% "C18"] 
## NOTE: This is called C20ArchRSubset to avoid making changes after adapting for the loop, but is a different cluster each time
...
#########################################
 # Use that to index which samples are being run
#  samp_comps_labs <- list() # make a list to put the non-empty comparisons into
#  empty_comps <- vector() # make a list to put the labels of empty comparisons into
#  error_comps <- vector() # make a list to put errors into
#  for(i in 1:nrow(non_redundant_combs)){ # for each row in the table of comparisons to make:
#    comparison_name <- paste(non_redundant_combs[i, 1], non_redundant_combs[i, 2], sep="_")  # make a name for the comparison by pasting together the first and second columns 
    
#    error_comps[[i]] <- tryCatch(
#      {
########################################
...
# Sort data matrix by log2FC values   
#sorted_markerList <- markerList[order(log2FC, decreasing = TRUE), ]

# Create heatmap using pheatmap
#pheatmap(sorted_data, cluster_rows = FALSE, cluster_cols = FALSE)
