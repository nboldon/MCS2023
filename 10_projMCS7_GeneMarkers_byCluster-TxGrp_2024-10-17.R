...
#################################################################
##  Get marker genes for each subset by cluster, groupBy Sample:
#################################################################
...
# 10-17-2024 cutOff = "FDR <= 0.01 & abs(Log2FC) >= 1.25"

#############################

# Write markerList to a CSV file
write.csv(markerList, file = "C3_markerList_2024-10-17.csv", row.names = FALSE)
...
for (cluster in clusters) {
  ...
  clusterData <- projMCS7[projMCS7$Clusters %in% cluster, ]
  ...
  # Wrap the getMarkerFeatures in a tryCatch to handle errors gracefully
  tryCatch({
    ...
    outputFile <- paste0(cluster, "_GeneMarker_groupBy-Sample_", Sys.Date(), ".csv")
    ...
    # If an error occurs, print the error message and move to the next cluster
    print(paste0("Error processing ", cluster, ": ", e$message))
  })
...
#################################################################
## Get marker genes for each subset by cluster, groupBy treatment:
## Loop for all clusters
#################################################################


## Note: groupBy treatment only returns: C18 (Gm4567), C21 (Olfr1337 & Olfr139)
...
for (cluster in clusters) {
  ...
  tryCatch({
    ...
    # Save markerList to a CSV file
    outputFile <- paste0(cluster, "_GeneMarker_groupBy-Treatment_", Sys.Date(), ".csv")
    write.csv(markerList, file = outputFile, row.names = FALSE)
...
#################################################################
##  Get marker genes for each subset by treatment, groupBy Clusters:
#################################################################
...
t1_markerList <- getMarkers(t1markers, cutOff = "FDR <= 0.01 & abs(Log2FC) >= 1.25", returnGR = TRUE)

t1_markerList

# 2024-10-17 cutOff: FDR <= 0.01 & abs(Log2FC) >= 1.25

t4_markerList$C1

#####

write.csv(t1_markerList, file = "T1_GeneMarker_groupBy-Treatment_2024-10-17.csv")
...
# List of treatment groups and corresponding sample names
treatment_groups <- list(
  t1 = c("C302_", "C306_", "C309_", "C318_", "C323_", "C328_", "C332_", "C337_", "C339_", 
         "C346_", "C351_", "C353_", "C360_"),
  t2 = c("C304_", "C308_", "C312_", "C315_", "C321_", "C324_", "C327_", "C330_", "C333_", 
         "C336_", "C342_", "C348_", "C349_", "C355_", "C358_", "C362_"),
  t3 = c("C305_", "C307_", "C313_", "C316_", "C320_", "C322_", "C325_", "C334_", "C340", 
         "C341_", "C345_", "C350_", "C352_", "C359_", "C364_"),
  t4 = c("C301_", "C303_", "C310_", "C314_", "C319_", "C335_", "C338_", "C344_", "C354_", 
         "C356_", "C361_", "C363")
)
...
for (treatment in names(treatment_groups)) {
  ...
  # Subset ArchR project for the current treatment group
  subsetArchR <- projMCS7[projMCS7$Sample %in% treatment_groups[[treatment]], ]
  
  # Get marker features for the current subset
  ...
  # Extract significant marker list
  markerList <- getMarkers(markerFeatures, cutOff = "FDR <= 0.01 & abs(Log2FC) >= 1.25")
  
  # Save the full marker list for the treatment group
  outputFileName <- paste0(treatment, "_GeneMarker_List_Master_", Sys.Date(), ".csv")
  write.csv(markerList, file = outputFileName)
  
  # Save individual cluster marker lists
  cluster_names <- names(markerList)  # Get cluster names (e.g., C1, C2, ...)
  
  # Loop through each cluster and save its specific marker list
  for (cluster in cluster_names) {
    clusterFileName <- paste0(treatment, "_", cluster, "_GeneMarker_List_", Sys.Date(), ".csv")
    write.csv(markerList[[cluster]], file = clusterFileName)
  }
  
  # Print message to track progress
  print(paste0("Processed treatment group: ", treatment))
}
...
# Get the names of all clusters (e.g., C1, C2, C3, ...)
cluster_names <- names(markerList)

# Loop through each cluster and save its marker list to a separate CSV file
for (cluster in cluster_names) {
  # Create a file name based on the cluster
  outputFileName <- paste0(cluster, "_GeneMarker_List_", Sys.Date(), ".csv")
  
  # Save the marker list for the current cluster to a CSV file
  write.csv(markerList[[cluster]], file = outputFileName)
  
  # Print message to track progress
  print(paste0("Saved marker list for cluster: ", cluster))
}
...
################################################################
######## Create dataframes for top genes by Log2FC #############
################################################################

# Using top genes from projMCS7, groupBy Clusters
# 2024-10-17 cutOff = "FDR <= 0.01 & abs(Log2FC) >= 1.25"
...
################################################################
########## Create heatmaps and print out zscores ###############
################################################################
...
## Get marker features
markerGS_C25 <- getMarkerFeatures(
...
#Get the list of markers
markerList_C3 <- getMarkers(markerGS_C3, cutOff = "FDR <= 0.01 & abs(Log2FC) >= 1.25")
...
se_idx <- which(rowData(markerGS_C25)$name %in% known_markers_C25)

#Get an index of these from the summarized experiment
markerGS_C25 <- markerGS_C25[se_idx,]

##Plot it out:

pdf(file="C25_TxComp_Heatmap_2024-10-18.pdf", width=15, height=30)
plotMarkerHeatmap(   ### Copied this over from the full function reference - leaving anything not commented at defaul$
  seMarker = markerGS_C25,   ## Set this to the correct object containing the markers
  ...
  clusterCols = TRUE,
  labelMarkers = known_markers_C25,  #Label the markers of interest we're plotting here
  nLabel = 15,
  nPrint = 15,
  labelRows = FALSE,
  returnMatrix = FALSE,
...
z_scores <- plotMarkerHeatmap(
  ...
  binaryClusterRows = TRUE,
  clusterCols = TRUE,
  labelMarkers = known_markers_C25,
  nLabel = 15,
  nPrint = 15,
  labelRows = FALSE,
...
write.csv(z_scores, "C25_byTx_zscores_2024-10-19.csv", row.names = TRUE)
...
# Read the data
mat <- read.csv("./C25_byTx_zscores_2024-10-18.csv", header = TRUE, row.names = 1, check.names = FALSE)
...
# Rename columns to match the treatment labels
colnames(sorted_mat) <- c("2N", "2N+", "Ts", "Ts+")

# Create a color palette for the heatmap
color_palette <- colorRampPalette(c("navy", "white", "red"))(50)

# Open a JPEG device for saving the plot
jpeg(file = "/Volumes/DataBox/MCS2023/Tx_Comp/C25_byTx_sorted_heatmap_by_t1_2024-10-18.jpg", width = 1200, height = 5000, res = 150)
...
pheatmap_result <- pheatmap(
  ...
  main = "Cluster 25 Heatmap of Gene Accessibility (Sorted by 2N)",
  color = color_palette,
  fontsize = 10, 
  cellwidth = 10, 
  cellheight = 10,
...
if (file.exists("/Volumes/DataBox/MCS2023/Tx_Comp/C3_byTx_sorted_heatmap_by_t1_2024-10-18.jpg")) {
  message("Heatmap saved successfully!")
} else {
  message("Error: Heatmap file not saved.")
}
...
# Open a JPEG device for saving the plot
jpeg(file = "/Volumes/DataBox/MCS2023/Tx_Comp/C25_byTx_sorted_heatmap_by_t3_2024-10-18.jpg", width = 1200, height = 5000, res = 150)
...
pheatmap_result <- pheatmap(
  ...
  cluster_rows = FALSE,  # Disable row clustering to show sorted order
  show_colnames = TRUE,  # Show column names
  main = "Cluster 25 Heatmap of Gene Accessibility (Sorted by Ts)",
  color = color_palette,
  fontsize = 10, 
  cellwidth = 10, 
  cellheight = 10,
...
## Uses the means of multiple rows being sorted
...
# Open a JPEG device for saving the plot
jpeg(file = "/Volumes/DataBox/MCS2023/Tx_Comp/C25_byTx_sorted_heatmap_by_t1-t2_2024-10-18.jpg", width = 1200, height = 5000, res = 150)
...
pheatmap_result <- pheatmap(
  ...
  cluster_rows = FALSE,  # Disable row clustering to show sorted order
  show_colnames = TRUE,  # Show column names
  main = "Cluster 25 Heatmap of Gene Accessibility (Sorted by 2N & 2N+)",
  color = color_palette,
  fontsize = 10, 
  cellwidth = 10, 
  cellheight = 10,
...
# Calculate the mean of t3 and t4 for sorting
mean_values <- rowMeans(mat[, c("t3", "t4")], na.rm = TRUE)

# Sort the data based on the mean of t3 and t4 values in descending order
sorted_mat <- mat[order(mean_values, decreasing = TRUE), ]
...
# Open a JPEG device for saving the plot
jpeg(file = "/Volumes/DataBox/MCS2023/Tx_Comp/C25_byTx_sorted_heatmap_by_t3-t4_2024-10-18.jpg", width = 1200, height = 5000, res = 150)
...
pheatmap_result <- pheatmap(
  ...
  cluster_rows = FALSE,  # Disable row clustering to show sorted order
  show_colnames = TRUE,  # Show column names
  main = "Cluster 25 Heatmap of Gene Accessibility (Sorted by Ts & Ts+)",
  color = color_palette,
  fontsize = 10, 
  cellwidth = 10, 
  cellheight = 10,
...
# Sort the data based on the mean of t2 and t4 values in descending order
sorted_mat <- mat[order(mean_values, decreasing = TRUE), ]
...
# Open a JPEG device for saving the plot
jpeg(file = "/Volumes/DataBox/MCS2023/Tx_Comp/C25_byTx_sorted_heatmap_by_t2-t4_2024-10-18.jpg", width = 1200, height = 5000, res = 150)
...
pheatmap_result <- pheatmap(
  ...
  cluster_rows = FALSE,  # Disable row clustering to show sorted order
  show_colnames = TRUE,  # Show column names
  main = "Cluster 25 Heatmap of Gene Accessibility (Sorted by 2N+ & Ts+)",
  color = color_palette,
  fontsize = 10, 
  cellwidth = 10, 
  cellheight = 10,
...
# Read the CSV file
data <- read.csv("C1_byTx_zscores_2024-10-18.csv")
...
write.csv(data, "C1_byTx_zscores_2024-10-18_analyzed.csv", row.names = FALSE)
...
## Loop for 2N/2N+ versus Ts/Ts+ 


# Loop through all clusters (1-25)
for (i in 1:25) {
  
  # Construct the filename for each cluster
  filename <- sprintf("C%d_byTx_zscores_2024-10-18.csv", i)
  ...
  # Save the updated data to a new CSV file
  output_filename <- sprintf("C%d_t1-t2_t3-t4_zscores_2024-10-18_analyzed.csv", i)
  write.csv(data, output_filename, row.names = FALSE)
  
  # Check for significant differences
  if (sum(data$"significant_(abs>=1.25)") == 0) {
    print(sprintf("Cluster %d: No significant differences", i))  # Print FALSE if no significant differences are found
  } else {
    # Extract and print the subset of data with significant differences
    significant_data <- data[data$"significant_(abs>=1.25)" == TRUE, ]
    print(sprintf("Cluster %d: Significant differences found", i))
    print(significant_data)
    
    # Optionally, save the significant data to a separate file
    significant_output_filename <- sprintf("C%d_significant_t1-t2_t3-t4_zscores_2024-10-18.csv", i)
...
for (i in 1:25) {
  ...
  # Save the updated data to a new CSV file
  output_filename <- sprintf("C%d_t2-t4_t1-t3_zscores_2024-10-18_analyzed.csv", i)
  write.csv(data, output_filename, row.names = FALSE)
  
  # Check for significant differences
  if (sum(data$"significant_(abs>=1.25)") == 0) {
    print(sprintf("Cluster %d: No significant differences", i))  # Print if no significant differences are found
  } else {
    # Extract and print the subset of data with significant differences
    significant_data <- data[data$"significant_(abs>=1.25)" == TRUE, ]
    print(sprintf("Cluster %d: Significant differences found", i))
    ...
    # Optionally, save the significant data to a separate file
    significant_output_filename <- sprintf("C%d_significant_t2-t4_t1-t3_zscores_2024-10-18.csv", i)
    write.csv(significant_data, significant_output_filename, row.names = FALSE)
  }
}
...
for (i in 1:25) {
  ...
  output_filename <- sprintf("C%d_t3_t4_zscores_2024-10-18_analyzed.csv", i)
  write.csv(data, output_filename, row.names = FALSE)
  ...
  if (sum(data$"significant_(abs>=1.25)") == 0) {
    ...
    # Optionally, save the significant data to a separate file
    significant_output_filename <- sprintf("C%d_significant_t3_t4_zscores_2024-10-18.csv", i)
    write.csv(significant_data, significant_output_filename, row.names = FALSE)
  }
}
...
# Initialize an empty data frame to store all the combined differences across clusters
...
for (i in 1:25) {
  ...
  # Select relevant columns for cluster differences (only significant ones)
  cluster_differences <- significant_data[, c("Gene", "difference_t3-t4")]
  
  # Rename the 'difference_group2-group1' column to reflect the cluster (e.g., C1, C2, etc.)
  colnames(cluster_differences)[2] <- sprintf("C%d_DA_Ts-Ts+", i)
  
  # Merge the current cluster's differences into the combined data frame
  if (nrow(combined_differences) == 0) {
    # If it's the first iteration, just assign it directly
    combined_differences <- cluster_differences
  } else {
    # Merge based on the 'Gene' column
    combined_differences <- merge(combined_differences, cluster_differences, by = "Gene", all = TRUE)
  }
  
  # Save the updated data for the current cluster
  #output_filename <- sprintf("C%d_t3-t4_zscores_2024-10-18_analyzed.csv", i)
  #write.csv(data, output_filename, row.names = FALSE)
}

# Export the combined data with only significant differences into a new spreadsheet
write.csv(combined_differences, "Combined_Cluster_significant_t3_t4_comparison_2024-10-18.csv", row.names = FALSE)
...
for (i in 1:25) {
  ...
  # Select relevant columns for cluster differences (only significant ones)
  cluster_differences <- significant_data[, c("Gene", "difference_group2-group1")]
  
  # Rename the 'difference_group2-group1' column to reflect the cluster (e.g., C1, C2, etc.)
  colnames(cluster_differences)[2] <- sprintf("C%d_DA_2N+/Ts+_2N/Ts", i)
...
# Export the combined data with only significant differences into a new spreadsheet
write.csv(combined_differences, "Combined_Cluster_significant_t2-t4_t1-t3_comparison_2024-10-18.csv", row.names = FALSE)
...
for (i in 1:25) {
  ...
  # Calculate difference between means
  data$"difference_group2-group1" <- data$"mean_group2_(t1+t2)/2" - data$"mean_group1_(t3+t4)/2"
  ...
  # Rename the 'difference_group2-group1' column to reflect the cluster (e.g., C1, C2, etc.)
  colnames(cluster_differences)[2] <- sprintf("C%d_DA_2N/2N+_Ts/Ts+", i)
...
# Export the combined data with only significant differences into a new spreadsheet
write.csv(combined_differences, "Combined_Cluster_significant_t1-t2_t3-t4_comparison_2024-10-18.csv", row.names = FALSE)
...
# Load your data
file_path <- "Combined_Cluster_significant_t3_t4_comparison_2024-10-18.csv"  # Update this to your file path
...
output_file_path <- "Combined_Cluster_Total-sig_t3_t4_comparison_2024-11-10.csv"  # Update this to your desired output path
...
file_path <- "Combined_Cluster_Total-sig_t1-t2_t3-t4_comparison_2024-10-18.csv"  # Update this to your file path
data <- read.csv(file_path)
...
## Tally total clusters that have the share DA gene markers between tx comops within cell types

## Cell Type IDs:
# Cluster 1 – Astrocyte (immature)
# Cluster 2 – Oligodendrocyte
# Cluster 3 – Oligodendrocyte 
# Cluster 4 – Glut-Oligo
# Cluster 5 – Oligo (immature)
# Cluster 6 – Oligo Precursor
# Cluster 7 – Astrocyte (immature) 
# Cluster 8 – Astrocyte
# Cluster 9 – Astrocyte (immature) 
# Cluster 10 - Microglia 
# Cluster 11 – Microglia
# Cluster 12 – Endo-vascular
# Cluster 13 – Glut (immature) 
# Cluster 14 – Endo-vascular
# Cluster 15 – Glut (immature) 
# Cluster 16 – Glut (immature) 
# Cluster 17 – Glut (immature)
# Cluster 18 - Glutamatergic
# Cluster 19 - Glutamatergic
# Cluster 20 – Glut (immature)
# Cluster 21 - Glutamatergic
# Cluster 22 - GABAergic (SST)
# Cluster 23 – GABAergic (VIP)
# Cluster 24 – Glut-Astro
# Cluster 25 – Glut (immature)

## Clusters grouped by cell type:

# Glutamatergic: 4, 13, 15, 16, 17, 18, 19, 20, 21, 24, 25
# GABAergic: 22, 23
# Oligodendrocytes: 2, 3, 5, 6
# Microglia: 10, 11
# Astrocytes: 1, 7, 8, 9
# Endo-vasc: 12, 14
...
# Define the cluster groupings
glutamatergic_clusters <- c("C4_DA_Ts_Ts.", "C13_DA_Ts_Ts.", "C15_DA_Ts_Ts.", "C16_DA_Ts_Ts.", "C17_DA_Ts_Ts.",
                            "C18_DA_Ts_Ts.", "C19_DA_Ts_Ts.", "C20_DA_Ts_Ts.", "C21_DA_Ts_Ts.", "C24_DA_Ts_Ts.", "C25_DA_Ts_Ts.")
gabaergic_clusters <- c("C22_DA_Ts_Ts.", "C23_DA_Ts_Ts.")
oligodendrocyte_clusters <- c("C2_DA_Ts_Ts.", "C3_DA_Ts_Ts.", "C5_DA_Ts_Ts.", "C6_DA_Ts_Ts.")
microglia_clusters <- c("C10_DA_Ts_Ts.", "C11_DA_Ts_Ts.")
astrocyte_clusters <- c("C1_DA_Ts_Ts.", "C7_DA_Ts_Ts.", "C8_DA_Ts_Ts.", "C9_DA_Ts_Ts.")
endo_vasc_clusters <- c("C12_DA_Ts_Ts.", "C14_DA_Ts_Ts.")

# Count non-NA values for each group
data_with_counts <- data %>%
  mutate(
    Glutamatergic_Count = rowSums(!is.na(select(., all_of(glutamatergic_clusters)))),
    GABAergic_Count = rowSums(!is.na(select(., all_of(gabaergic_clusters)))),
    Oligodendrocytes_Count = rowSums(!is.na(select(., all_of(oligodendrocyte_clusters)))),
    Microglia_Count = rowSums(!is.na(select(., all_of(microglia_clusters)))),
    Astrocytes_Count = rowSums(!is.na(select(., all_of(astrocyte_clusters)))),
    Endo_Vasc_Count = rowSums(!is.na(select(., all_of(endo_vasc_clusters))))
  )

# Calculate total significant entries for each group (summing non-NA counts across all genes)
total_counts <- data_with_counts %>%
  summarise(
    Glutamatergic_Total = sum(Glutamatergic_Count),
    GABAergic_Total = sum(GABAergic_Count),
    Oligodendrocytes_Total = sum(Oligodendrocytes_Count),
    Microglia_Total = sum(Microglia_Count),
    Astrocytes_Total = sum(Astrocytes_Count),
    Endo_Vasc_Total = sum(Endo_Vasc_Count)
  )

# Print total counts for each group
print(total_counts)

#   Glutamatergic_Total GABAergic_Total Oligodendrocytes_Total Microglia_Total Astrocytes_Total
#                  770             181                    336             187              333
# Endo_Vasc_Total
#             224

# Save the detailed data with counts as a new CSV file
output_file_path <- "Cluster_DA-Gene_Counts_byCellType_t3-t4_comparison_2024-11-10.csv"  # Update this to your desired output path
write.csv(data_with_counts, output_file_path, row.names = FALSE)
...
## Create a barplot to display cell type tallies for total number of DA genes observed
...
# Convert total_counts to a long format data frame for ggplot2
total_counts_long <- total_counts %>%
  pivot_longer(cols = everything(), names_to = "Cluster_Group", values_to = "Count")

# Create the barplot
plot <- ggplot(total_counts_long, aes(x = Cluster_Group, y = Count, fill = Cluster_Group)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d(option = "viridis", name = "Cell Type Group") +  # Use Viridis color palette
  labs(
    title = "Total DA Gene Counts per Cell Type Group - T3-T4 Comparison",
    x = "Cell Type Group",
    y = "Total DA Gene Count"
  ) +
  theme_minimal() +
...
ggsave("Cluster_TOTAL-DA-Gene_Counts_byCellType_t3-t4_comparison_2024-11-10.jpg", plot = plot, width = 8, height = 6, dpi = 300)
...
## Create a barplot to display cell type tallies for number of DA genes observed in 1 cluster, 2 clusters, or 3 clusters
...
# Load your data
file_path <- "Combined_Cluster_significant_t3_t4_comparison_2024-10-18.csv"  # Update to your file path
data <- read.csv(file_path)

# Define cluster groupings
groups <- list(
  Glutamatergic = c("C4_DA_Ts_Ts.", "C13_DA_Ts_Ts.", "C15_DA_Ts_Ts.", "C16_DA_Ts_Ts.", 
                    "C17_DA_Ts_Ts.", "C18_DA_Ts_Ts.", "C19_DA_Ts_Ts.", "C20_DA_Ts_Ts.",
                    "C21_DA_Ts_Ts.", "C24_DA_Ts_Ts.", "C25_DA_Ts_Ts."),
  GABAergic = c("C22_DA_Ts_Ts.", "C23_DA_Ts_Ts."),
  Oligodendrocytes = c("C2_DA_Ts_Ts.", "C3_DA_Ts_Ts.", "C5_DA_Ts_Ts.", "C6_DA_Ts_Ts."),
  Microglia = c("C10_DA_Ts_Ts.", "C11_DA_Ts_Ts."),
  Astrocytes = c("C1_DA_Ts_Ts.", "C7_DA_Ts_Ts.", "C8_DA_Ts_Ts.", "C9_DA_Ts_Ts."),
  Endo_vasc = c("C12_DA_Ts_Ts.", "C14_DA_Ts_Ts.")
)

# Initialize an empty data frame to store the results
tally_results <- data.frame()

# Loop through each cell type group to calculate the counts
for (group_name in names(groups)) {
  # Select only the columns for the current cell type group
  group_data <- data %>%
    select(Gene, all_of(groups[[group_name]]))
  
  # Count the number of non-NA values per gene within this group
  group_data <- group_data %>%
    mutate(Count_NonNA = rowSums(!is.na(select(., -Gene))))
  
  # Tally how many genes have exactly 1, 2, 3, etc., clusters
  group_tally <- group_data %>%
    count(Count_NonNA) %>%
    filter(Count_NonNA > 0)  # Exclude rows with 0 clusters
  
  # Add the cell type group name to the tally data
  group_tally$Cluster_Group <- group_name
  
  # Append the tally to the results data frame
  tally_results <- bind_rows(tally_results, group_tally)
}

# Rename columns for clarity
colnames(tally_results) <- c("Clusters_Per_Gene", "Gene_Count", "Cluster_Group")

# Create the barplot
plot <- ggplot(tally_results, aes(x = Clusters_Per_Gene, y = Gene_Count, fill = Cluster_Group)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_viridis_d(option = "viridis", name = "Cell Type Group") +  # Use Viridis color palette
  labs(
    title = "Number of DA Genes Appearing in Multiple Clusters per Cell Type Group - T3 vs T4 Comp",
    x = "Number of Clusters per DA Gene",
    y = "DA Gene Count"
  ) +
  theme_minimal() + 
...
# Save the plot as a JPG file
ggsave("Gene_Counts_by_Cluster_t3-t4-comp_CellTypeID_2024-11-10.jpg", plot = plot, width = 8, height = 6, dpi = 300)
...
## Load known_marker lists for each cluster



# Function to create a cluster subset
createClusterSubset <- function(proj, cluster) {
  return(proj[proj$Clusters == cluster])
}

# Function to process a single cluster
processCluster <- function(clusterObj, clusterNum, known_markers) {
  # Get marker features
  markerGS <- getMarkerFeatures(
    ArchRProj = clusterObj,
    useMatrix = "GeneScoreMatrix",
    groupBy = "treatment",
    testMethod = "wilcoxon",
    bias = c("TSSEnrichment", "log10(nFrags)"),
  ...
  # Subset to known markers
  se_idx <- which(rowData(markerGS)$name %in% known_markers)
  markerGS <- markerGS[se_idx,]
  
  # Create heatmap
  pdf(file = sprintf("C%d_TxComp_Heatmap_%s.pdf", clusterNum, format(Sys.Date(), "%Y-%m-%d")), 
      width = 15, height = 30)
  plotMarkerHeatmap(
    seMarker = markerGS,
    cutOff = "FDR <= 1 & Log2FC >= 0.01",
  ...
  # Save z-scores
  write.csv(z_scores, 
            sprintf("C%d_byTx_zscores_%s.csv", 
                    clusterNum, format(Sys.Date(), "%Y-%m-%d")), 
            row.names = TRUE)
  
  # Create sorted heatmaps
  createSortedHeatmaps(z_scores, clusterNum)
}

# Function to create sorted heatmaps
createSortedHeatmaps <- function(z_scores_file, clusterNum) {
  # Load z-scores
  mat <- as.matrix(read.csv(z_scores_file, header = TRUE, row.names = 1, check.names = FALSE))
  colnames(mat) <- c("2N", "2N+", "Ts", "Ts+")
  
  # Define sorting columns
  sort_cols_list <- list(c("t1"), c("t3"), c("t1", "t2"), c("t3", "t4"))
  titles <- c(
    sprintf("Cluster %d Heatmap of Gene Accessibility (Sorted by 2N)", clusterNum),
    sprintf("Cluster %d Heatmap of Gene Accessibility (Sorted by Ts)", clusterNum),
    sprintf("Cluster %d Heatmap of Gene Accessibility (Sorted by 2N & 2N+)", clusterNum),
    sprintf("Cluster %d Heatmap of Gene Accessibility (Sorted by Ts & Ts+)", clusterNum)
  )
  
  # Create sorted heatmaps
  for (i in seq_along(sort_cols_list)) {
    sorted_mat <- mat[order(rowMeans(mat[, sort_cols_list[[i]], drop = FALSE], na.rm = TRUE), decreasing = TRUE), ]
    jpeg(file = sprintf("/Volumes/DataBox/MCS2023/Tx_Comp/C%d_byTx_sorted_heatmap_%s.jpg", 
                        clusterNum, format(Sys.Date(), "%Y-%m-%d")), width = 1200, height = 5000, res = 150)
    pheatmap(
      sorted_mat,
      scale = "none",
      cluster_cols = FALSE,
...
# Main loop to process all clusters
for (i in 1:25) {
  # Create cluster subset
  clusterObj <- createClusterSubset(projMCS7, sprintf("C%d", i))
  
  # Get known markers for this cluster
  known_markers <- get(sprintf("known_markers_C%d", i))
  
  # Process the cluster
  tryCatch({
    message(sprintf("Processing Cluster %d...", i))
    processCluster(clusterObj, i, known_markers)
    message(sprintf("Cluster %d completed successfully", i))
  }, error = function(e) {
    message(sprintf("Error processing Cluster %d: %s", i, e$message))
  })
}
...
other attached packages:
 [1] nabor_0.5.0                 patchwork_1.1.3            
 [3] aplot_0.2.2                 ggtree_3.8.2               
 [5] cowplot_1.1.2               tidyr_1.3.1                
 [7] dplyr_1.1.4                 BiocManager_1.30.22        
 [9] Signac_1.12.0               Seurat_5.0.1               
[11] SeuratObject_5.0.1          sp_2.1-2                   
[13] pheatmap_1.0.12             enrichplot_1.20.3          
[15] clusterProfiler_4.8.3       org.Mm.eg.db_3.17.0        
[17] AnnotationDbi_1.64.1        rhdf5_2.44.0               
[19] SummarizedExperiment_1.32.0 Biobase_2.62.0             
[21] MatrixGenerics_1.14.0       Rcpp_1.0.12                
[23] Matrix_1.6-4                GenomicRanges_1.54.1       
[25] GenomeInfoDb_1.38.5         IRanges_2.36.0             
[27] S4Vectors_0.40.2            BiocGenerics_0.48.1        
[29] matrixStats_1.2.0           data.table_1.14.10         
[31] stringr_1.5.1               plyr_1.8.9                 
[33] magrittr_2.0.3              ggplot2_3.4.4              
[35] gtable_0.3.4                gtools_3.9.5               
...
loaded via a namespace (and not attached):
 ...
 [69] rhdf5filters_1.12.1                fansi_1.0.6                       
 [71] digest_0.6.33                      R6_2.5.1                          
 [73] mime_0.12                          gridGraphics_0.5-1                
 [75] colorspace_2.1-0                   scattermore_1.2                   
 [77] GO.db_3.18.0                       Cairo_1.6-2                       
 [79] tensor_1.5                         spatstat.data_3.0-3               
 [81] RSQLite_2.3.5                      utf8_1.2.4                        
 [83] generics_0.1.3                     rtracklayer_1.62.0                
 [85] htmlwidgets_1.6.4                  graphlayouts_1.0.2                
 [87] httr_1.4.7                         S4Arrays_1.2.0                    
 [89] scatterpie_0.2.1                   uwot_0.1.16                       
 [91] pkgconfig_2.0.3                    blob_1.2.4                        
 [93] lmtest_0.9-40                      XVector_0.42.0                    
 [95] shadowtext_0.1.2                   htmltools_0.5.7                   
 [97] dotCall64_1.1-1                    fgsea_1.26.0                      
 [99] scales_1.3.0                       png_0.1-8                         
[101] ggfun_0.1.3                        rjson_0.2.21                      
[103] reshape2_1.4.4                     nlme_3.1-164                      
[105] cachem_1.0.8                       zoo_1.8-12                        
[107] KernSmooth_2.23-22                 miniUI_0.1.1.1                    
[109] HDO.db_0.99.1                      restfulr_0.0.15                   
[111] pillar_1.9.0                       vctrs_0.6.5                       
[113] RANN_2.6.1                         promises_1.2.1                    
[115] xtable_1.8-4                       cluster_2.1.4                     
[117] Rsamtools_2.18.0                   cli_3.6.2                         
[119] compiler_4.3.2                     rlang_1.1.3                       
[121] crayon_1.5.2                       future.apply_1.11.0               
[123] fs_1.6.3                           stringi_1.8.3                     
[125] deldir_1.0-9                       viridisLite_0.4.2                 
[127] BiocParallel_1.36.0                munsell_0.5.0                     
[129] Biostrings_2.70.1                  lazyeval_0.2.2                    
[131] spatstat.geom_3.2-7                GOSemSim_2.26.1                   
[133] BSgenome_1.70.1                    RcppHNSW_0.5.0                    
[135] bit64_4.0.5                        future_1.33.0                     
[137] Rhdf5lib_1.22.1                    KEGGREST_1.42.0                   
[139] shiny_1.8.0                        ROCR_1.0-11                       
[141] igraph_1.5.1                       memoise_2.0.1                     
[143] fastmatch_1.1-4                    bit_4.0.5                         
[145] downloader_0.4                     BSgenome.Mmusculus.UCSC.mm10_1.4.3
[147] ape_5.7-1                          gson_0.1.0   
