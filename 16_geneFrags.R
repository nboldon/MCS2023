...
sample_names <- unique(projMCS1$Sample)
...
write.csv(results, "gene_sample_statistics_MCS1_2024-09-12.csv", row.names = FALSE)
...
for (region_name in names(regions_test)) {
  ...
  region <- regions_test[[region_name]]
  ...
  for (i in sample_names) {
    ...
    geneRanges <- rowRanges(geneMatrix)
    
    # Subset the peak ranges by the genomic region of interest
    subset_genes <- subsetByOverlaps(geneRanges, region)
    
    # Get the indices of the overlapping peaks
    indices <- subset_genes$idx
    
    # Subset the peak matrix using these indices
    subset_fragments <- geneMatrix[indices, , drop = FALSE]
  ...
  colnames(results)[2] <- paste0(region_name, "geneFrags")
  
  # Write the results to a CSV file named after the region
  write.csv(results, paste0(region_name, "_geneFrag_counts_2024-09-12.csv"), row.names = FALSE)
}
