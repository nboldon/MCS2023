...
file_path <- "/Volumes/DataBox/GO_Analysis/GO_Genes-Of-Interest_t2-t4_t1-t3_2024-11-08.csv"
...
write.csv(duplicate_genes, file = paste0("GO-BP-GenesOfInterest_Duplicate_Genes_t2-t4_t1-t3_", date_today, ".csv"), row.names = FALSE)

# Plot the data and save as a JPG
output_file <- paste0("Cluster_Gene_Counts_t2-t4_t1-t3_", date_today, ".jpg")
plot <- ggplot(gene_counts, aes(x = Cluster, y = Gene_Count)) +
  geom_bar(stat = "identity", fill = "#440154") +
  theme_minimal() +
  labs(title = "GO BP Genes of Interest Counts by Cluster - 2N+/Ts+ vs. 2N/Ts",
       x = "Cluster",
...
ggsave(output_file, plot = plot, width = 7, height = 6, dpi = 300)
...
file_path <- "/Volumes/DataBox/GO_Analysis/GO_Genes-Of-Interest_t3_t4_2024-11-08.csv"
    ...
    if (nrow(entrez_genes) > 0) {
      ...
      if (!is.null(go_enrichment) && nrow(go_enrichment) > 0) {
        ...
        go_plot <- barplot(go_enrichment, showCategory = 10) + 
          ...
          scale_fill_viridis_c()  # Apply continuous viridis color scale to the bars
        
        # Save the plot as a JPG file with adjusted size (widening the plot)
        plot_file <- paste0(col_name,"_GO_GenesOfInterest_byCluster_Barplots_t3_t4_2024-12-16.jpg")
  ...
  if (!is.null(entrez_genes) && nrow(entrez_genes) > 0) {
    ...
    if (nrow(entrez_genes) > 0) {
      ...
      if (!is.null(go_enrichment) && nrow(go_enrichment) > 0) {
        ...
        plot_file <- paste0(col_name, "_GO_GenesOfInterest_Dotplot_t1-t2_t3-t4_2024-12-16.png")
